...
The CSV files use UTF-8 encoding, comma delimiters, and a single header row. Identifiers are strings. Numerical values use a decimal point; empty fields denote unavailable or inapplicable entries, never zero. Retained numerical precision reflects the compilation and conversions, not necessarily the precision of the original measurements. Source identifiers are stable, so gaps in their numbering are intentional.
